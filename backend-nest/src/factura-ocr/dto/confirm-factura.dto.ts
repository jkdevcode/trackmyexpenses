...
const facturaConfirmadaSchema = z.object({
  factura: z.object({
    ...
    metodoPago: z.enum(['EFECTIVO', 'TARJETA_CREDITO', 'TARJETA_DEBITO', 'TRANSFERENCIA', 'OTRO']).default('EFECTIVO'),
  ...
  productos: z.array(productoConfirmadoSchema).min(1, { message: 'Debe haber al menos un producto' })
});

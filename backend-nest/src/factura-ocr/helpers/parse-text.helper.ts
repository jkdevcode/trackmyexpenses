export class TextParserHelper {
  ...
  static detectWeightOrUnit(line: string): { cantidad: number, unidad: 'u'|'kg'|'g' } {
      // Regex for weight: number followed by kg/g/etc
      // "1.5 kg", "0,500 KG", "2 KILOS"
      const weightRegex = /(\d+[.,]?\d*)\s*(kg|kgs|kilos|kilogramos|g|gr|gramos|lb|libras)/i;
      const match = line.match(weightRegex);

      if (match) {
          // Normalize number first
          let qty = this.normalizeNumber(match[1]);
   ...
   */
  static fallbackParse(text: string): any[] {
    ...
    for (const line of lines) {
        ...
        if (cleanLine.match(/(fecha|total|subtotal|iva|cambio|efectivo|nit|factura)/i)) continue;
        ...
        if (match) {
            ...
            const unitPrice = unitInfo.cantidad > 0 ? Math.round(total / unitInfo.cantidad) : total;

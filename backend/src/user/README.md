...
- Persistir cambios de usuario, foto y `monedaBase` mediante Prisma/storage.

## Main Files

- **`user.controller.ts`**: Endpoints `/users` y reglas de autorizacion.
- **`user.service.ts`**: Logica de negocio para perfil, moneda base y contrasena.
...
- Invocado por frontend para hydration de sesion (`/users/me`) y actualizacion de perfil/settings.

            ...
            >
                <TableHeader columns={columns}>
                    {(column) => (
                        <TableColumn
                            key={column.key}
                            className={column.key === "status" || column.key === "date" ? "hidden sm:table-cell" : ""}
                        >
                            {column.label}
                        </TableColumn>
                    )}
                                        ...
                                        return (
                                            <TableCell>
                                                <div className="flex flex-col">
                                                    <span className="font-medium text-foreground text-sm truncate max-w-[120px] sm:max-w-xs" title={item.provider}>
                                                        {item.provider}
                                                    </span>
                                                    ...
                                                    <div className="flex sm:hidden gap-2 text-xs text-default-400">
                                                        ...
                                                        <span>•</span>
                                                        <span className={`text-${statusColorMap[item.status]}`}>{item.status}</span>
                                                    </div>
                                                </div>
                                            </TableCell>
                                        );
                                    case "date":
                                        return (
                                            <TableCell className="hidden sm:table-cell whitespace-nowrap">
                                                <span className="text-default-500 text-sm">{formatDate(item.date)}</span>
                                            </TableCell>
                                        );
                                    case "total":
                                        return (
                                            <TableCell>
                                                <span className={`font-semibold text-${appColor} whitespace-nowrap`}>

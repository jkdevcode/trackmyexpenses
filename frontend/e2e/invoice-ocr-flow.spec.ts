...
test("OCR flow renders structured backend item errors", async ({ page }) => {
  test.setTimeout(120_000);
  ...
  await page.route("**/facturas/ocr/create", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        success: false,
        error: {
          code: "FACTURA_OCR_ITEM_PRECIO_INVALID",
          details: [
            {
  ...
  await expect(page.getByText(/1 item\(s\) have errors/i).first()).toBeVisible({
    timeout: 15_000,
  });
  await expect(
    page.getByText(/Price could not be detected for this item/i),
  ).toBeVisible({ timeout: 15_000 });
